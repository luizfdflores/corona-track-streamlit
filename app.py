...
country = st.selectbox("Enter country", sorted(list(countryDict.keys())), )
...
if st.checkbox('Show raw data'):
    ...
    st.dataframe(data[['name', 'population', 'today.deaths', 'today.confirmed', 'date', 'deaths', 'confirmed', 'active',
                       'recovered']])
...
if expr2 > 0:
    ...
    st.write('\tR^2:', expr2)
    st.write('\tDoubling Time (represents overall growth): ', round(edoubletime, 2), '(±', round(edoubletimeerror, 2),
             ') days')

if logisticr2 > 0:
    st.write('\n** Based on Logistic Fit**\n')
    st.write('\tR^2:', logisticr2)
    st.write('\tDoubling Time (during middle of growth): ', round(ldoubletime, 2), '(±', round(ldoubletimeerror, 2),
             ') days')
